...
def start_capture(input_path,output_path):
        # path = "./data1/unknow/5"
        num_of_images = 0
        ...
        with mp_hands.Hands(
            ...
            while cap.isOpened():
                num_of_images = len([entry for entry in os.listdir(output_path) if os.path.isfile(os.path.join(output_path, entry))]) + 1
                    ...
                    try:
                        ...
                        if results.multi_hand_landmarks:
                            ...
                            for hand_landmark in results.multi_hand_landmarks:
                                ...
                                # roi_img1=cv2.rectangle(roi_img, (lxROI, lyROI),
                                #     (uxROI, uyROI), (0, 255, 0), 2)

                                # cv2.imshow("roi_zone_img", roi_img1)
                                ...
                                cv2.imwrite(str(output_path + "/" + str(num_of_images) + ".bmp"), roi_img)
                                # print(valueOfImage)
...
input_subdirs = os.listdir(input_dir)

for subdir in input_subdirs:
  input_path = os.path.join(input_dir, subdir)
  
  # Lấy tên thư mục con 
  subdir_name = os.path.basename(input_path)
  ...
  # Chạy xử lý trên thư mục con
    start_capture(os.path.join(input_path, video), output_path)    
# start_capture()